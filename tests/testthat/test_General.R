...
test_that('Parametrization and its inverse works', {
  ...
  mfdList <- RFPCA:::listAvailMfd()
  mfdFinite <- mfdList[vapply(mfdList, RFPCA:::is.finiteDim, TRUE)]
  ...
  for (mfdName in mfdFinite) {
    ...
    mfd <- RFPCA::createM(mfdName)
    dInt <- RFPCA::calcIntDim(mfd, geomPar=p)

    data <- t(RFPCA::rmfd(mfd, n, dInt))

    basis <- RFPCA::basisTan(mfd, data[1, ])
